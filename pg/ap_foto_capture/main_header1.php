...
<section class="content-header">
    ...
    <div class="box box-primary">
        ...
        <div class="row">
          ...
          $sql = "SELECT * FROM ds_foto ORDER BY id_foto ASC LIMIT 1";
...
<div class="row">
                <form action="index.php" method="GET" id="combo">
                <div class="col-md-3">
                  ...
                  <?php 
                if (isset($_POST["label_gambar"])) {
                    $label = $_POST["label_gambar"];
                    require_once "db_con.php";
                    ini_set('date.timezone', 'Asia/Jakarta');
                    $t_taken = new DateTime();
                    $begin = time();
                    $tanggal = $t_taken->format("Y-m-d H:i:s");
                    $command = escapeshellcmd("motret_realsense.py $label");
                    ...
                    while (1) {                        
                        if (time() - $begin < 15) {
                            if ($output == "") {
                                ...
                                if ($output != "end") {
                                    //echo $output;
                                    $conn = new mysqli('localhost', 'root', '', 'db_camera');
                                    $query = "SELECT file_path FROM data_gambar WHERE label='$label'";
                                    $result = $conn->query($query);
                                    while ($row = $result->fetch_assoc()) {
                                        $file_path = $row['file_path'];
                                    }
                                    echo "<div class='text-center mt-4'><br><br/><td><img src='$file_path' alt='$label' width='800'></td></div>";
                                    break;
                                }
                                else {
