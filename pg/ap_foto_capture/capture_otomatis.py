...
def capture() : 
    ...
    val = (layout, kamera, data_tanggal, data_waktu, img_nama, t1, t2, t3, t5, t6, t7, t8, t9, t10)
...
def schedule_pagi() :
    capture()
    schedule.every(2).minutes.until("10:00").do(capture)
    return schedule.CancelJob

def schedule_siang() :
    capture()
    schedule.every(2).minutes.until("15:00").do(capture)
    return schedule.CancelJob

def schedule_sore() :
    capture()
    schedule.every(2).minutes.until("19:00").do(capture)
    return schedule.CancelJob

# Fungsi otomasi 
schedule.every().day.at("05:00").do(schedule_pagi)
schedule.every().day.at("13:00").do(schedule_siang)
schedule.every().day.at("15:00").do(schedule_sore)

# schedule.every(5).minutes.do(capture)
